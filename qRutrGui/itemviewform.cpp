...
void ItemViewForm::replaceRegExp(QRegExp &exp, QString &res, QString &str)
{
    int pos = 0;
    while (pos >= 0)
    {
        ...
        if (pos >= 0)
        {
            QString s = str.arg(exp.cap(1));
            res = res.remove(pos, exp.matchedLength());
...
QString ItemViewForm::GetHTML(QString content)
{
    ...
    QString str = "<var class=\"postImg\" title=\"%1\"><img src=\"%1\" class=\"postImg\" alt=\"pic\"></var>";
    replaceRegExp(rx, res, str);

    rx.setPattern("\\[img=right\\](.+)\\[/img\\]");
    rx.setMinimal(true);
    rx.setCaseSensitivity(Qt::CaseInsensitive);
    str = "<var class=\"postImg postImgAligned img-right\" title=\"%1\"><img src=\"%1\" class=\"postImg postImgAligned img-right\" alt=\"pic\"></var>";
    replaceRegExp(rx, res, str);

    rx.setPattern("\\[spoiler=\"(.+)\"\\]");
    rx.setMinimal(true);
    rx.setCaseSensitivity(Qt::CaseInsensitive);
    str = "<div class=\"sp-wrap clearfix sp-opened\"> <div class=\"sp-head folded unfolded\"><span>%1</span></div><div class=\"sp-body clearfix inited\" style=\"display: block;\">";
    replaceRegExp(rx, res, str);

    rx.setPattern("\\[quote=\"(.+)\"\\]");
    rx.setMinimal(true);
    rx.setCaseSensitivity(Qt::CaseInsensitive);
    str = "<div class=\"q-wrap\"><div class=\"q-head\"><span><b>%1</b> писал(а):</span></div><div class=\"q\">";
    replaceRegExp(rx, res, str);

    rx.setPattern("\\[font=\"(.+)\"\\]");
    rx.setMinimal(true);
    rx.setCaseSensitivity(Qt::CaseInsensitive);
    str = "<span class=\"post-font-%1\">";
    replaceRegExp(rx, res, str);
